...
| 0.8.1 | Phase 5's pull half (2) — availability read back via `freebusy.query`, which needs a cron-refreshed busy cache rather than a live call inside the request; plus the server noticeboard (5, per `specs/0007`) and the sandbox frontend gap closed as decided-against (23). Empties `IDEAS.md`'s **Still open**, so 1.0 follows once the security review is done | In progress |
